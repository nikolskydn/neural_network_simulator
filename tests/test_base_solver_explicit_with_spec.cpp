...
BOOST_AUTO_TEST_CASE (testForExplicitSolverWithSpecElems)
{
    ...
    inFBuff << numberSolver << simulationTime << dt;
    // neurs
    inFBuff << neursType << nNeurs << t 
           << V[0] << V[1] << V[2] 
           << mask[0] << mask[1] << mask[2] 
           << VPeak  << VReset << paramSpecNeurs ;

    // conns
    inFBuff << connsType << nConns << t
            << I[0] << I[1] << I[2]
            << W[0] << W[1] << W[2]
            << W[3] << W[4] << W[5]
            << W[6] << W[7] << W[8]
     ...
     * {
     *     V[mask] = VReset
     *     V[!mask] += paramSpecNeurs*dt*I[!mask]
     *     mask = V>VPeak
     * }
     */
    ...
    size_t outNeursType;
    outBuff >> outNeursType;
    BOOST_CHECK( outNeursType == neursType ); 
     ...
     * {
     *     I[!mask] *= 0.5
     *     I[mask] += paramSpecConns*dt*V[mask]
     * }
     */
    
    size_t outConnsType;
    outBuff >> outConnsType;
    BOOST_CHECK( outConnsType == connsType ); 

    size_t outNConns;
    outBuff >> outNConns;
    BOOST_CHECK( outNConns == nConns ); 
